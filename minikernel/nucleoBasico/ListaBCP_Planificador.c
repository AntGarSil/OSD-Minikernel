...
TipoBCP * planificador()
{
	...
	/*Si hay procesos con prioridad alta, se devuelven estos primero*/	
	procesoAux = primeroListaBCP(&listaListosAltaPrioridad);
	if (procesoAux != NULL){
		eliminarPrimeroListaBCP(&listaListosAltaPrioridad);
		return (procesoAux);	
	}
	/*Si no, se busca en la lista de procesos con prioridad normal*/
	else{
	...
	/*Si no hay procesos en ninguna lista, se devuelve NULL*/
		else{
			return (NULL);
		}
	}
